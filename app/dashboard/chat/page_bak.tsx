...
const AgenticChat: React.FC = () => {
  ...
  return (
    ...
    >
      <ThreadedChat threadsKey={threadsKey} user={user}/>
    </CopilotKit>
  );
};
...
const ThreadedChat: React.FC<{ threadsKey: string, user: UserResponse }> = ({ threadsKey, user }) => {
  // Manage activeThreadId locally instead of using useCopilotContext
  ...
  // Local state for active thread id
  const [activeThreadId, setActiveThreadId] = useState(() => threads[0]?.id);

  // Keep activeThreadId in sync with threads
  useEffect(() => {
                ...
                <div className="flex items-center gap-1">
                  {inProgress && onStop ? (
                    ...
                    <button
                      ...
                      className="p-2 text-blue-500 hover:bg-blue-50 disabled:text-gray-300 disabled:hover:bg-transparent rounded-lg transition-colors cursor-pointer"
                    >
